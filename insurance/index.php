...
// Get assigned claims
$result = $conn->query("SELECT COUNT(*) as count FROM accident_report WHERE assigned_adjuster = $user_id AND status IN ('Assigned', 'UnderReview')");
if ($result && $row = $result->fetch_assoc()) {
    $stats['assigned_claims'] = $row['count'];
}

// Get reviewed claims this month
$result = $conn->query("SELECT COUNT(*) as count FROM accident_report WHERE assigned_adjuster = $user_id AND status IN ('Approved', 'Rejected') AND MONTH(reviewed_at) = MONTH(CURRENT_DATE())");
if ($result && $row = $result->fetch_assoc()) {
    $stats['reviewed_claims'] = $row['count'];
}

// Get pending approval
$result = $conn->query("SELECT COUNT(*) as count FROM accident_report WHERE assigned_adjuster = $user_id AND status = 'UnderReview'");
if ($result && $row = $result->fetch_assoc()) {
    $stats['pending_approval'] = $row['count'];
}

// Get approved amount this month
$result = $conn->query("SELECT SUM(approved_amount) as total FROM accident_report WHERE assigned_adjuster = $user_id AND status = 'Approved' AND MONTH(reviewed_at) = MONTH(CURRENT_DATE())");
if ($result && $row = $result->fetch_assoc()) {
    $stats['approved_amount'] = $row['total'] ?: 0;
}
...
$assigned_claims = $conn->query("SELECT ar.*, u.first_name, u.last_name, c.make, c.model, c.number_plate, p.policy_number 
                                ...
                                WHERE ar.assigned_adjuster = $user_id 
    ...
    <style>
        * {
        ...
        .sidebar-menu a.active {
            ...
            color: white;
            border-left: 4px solid var(--adjuster-color);
        }
        ...
        .stat-icon.adjuster { background-color: var(--adjuster-color); }
        ...
        <main class="main-content">
            <div class="content-header">
                ...
                <div class="header-actions">
                    <a href="assessments.php" class="btn btn-adjuster">
            ...
            <div class="dashboard-stats">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-details">
                        <h3><?php echo number_format($stats['reviewed_claims']); ?></h3>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-details">
                        <h3><?php echo format_currency($stats['approved_amount']); ?></h3>
                        <p>Approved Amount</p>
                    </div>
                </div>
            </div>
                        ...
                        <tbody>
                            <?php while ($claim = $assigned_claims->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="view-claim.php?id=<?php echo $claim['Id']; ?>" class="btn">Review</a>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
